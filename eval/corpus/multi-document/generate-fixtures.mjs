/**
 * generate-fixtures.mjs — Generates the 30 physical fixtures for M9.1.
 */

import { writeFileSync, mkdirSync, rmSync } from 'node:fs';
import { join, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { M9_CORPUS_ITEMS } from './corpus-data.mjs';
...
console.log('Generating M9.1 Feasibility Corpus (N=30)...');
...
for (const item of M9_CORPUS_ITEMS) {
  ...
  inventory.push({
    ...
    fixture_path: `eval/corpus/multi-document/fixtures/${item.fixture_id}`,
  });
}

writeFileSync(INVENTORY_PATH, JSON.stringify(inventory, null, 2), 'utf8');
console.log(`✓ Successfully generated 30 fixtures in ${FIXTURES_DIR}`);
